...
| `public/icon-source.png` | Ray's artwork, 1024², the one source of truth |
...
**The icons are opaque.** Every size is flattened onto the paper background
(`#e9e0cd`), including the plain ones. An app icon is never seen on nothing — it
sits on a dock, a tab strip, a home screen — so a transparent one picks up
whatever is behind it and the card reads as a cutout on a dark dock, with its
drop shadow falling on air. The cream tile is the icon's own surface.
