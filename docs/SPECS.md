...
| `public/icon.svg`, `public/icon-maskable.svg` | The two artwork sources |
| `public/*.png` | Generated by `scripts/make-icons.sh`, committed |
...
Regenerate the PNGs with `scripts/make-icons.sh` after editing either SVG, and
commit what it writes. It needs ImageMagick and Chrome, and never runs during a
build or a deploy.
