...
| Detail | Manuscript: write inside the chapter | ✅ | **The prose lives in the map.** A `Manuscript` section under Scene flow, in three states (`manuscriptState`, persisted): **Minimized** — a dashed row reporting `N scenes · N words written`, so a planning session never sees prose; **Regular** — the writing sheet opens and the scene flow becomes a **guide**: a sticky, horizontally scrolling strip of the chapter's beats with their connector pills, above the pane; **Full screen** — the same, filling the viewport with everything below the sheet dropped. The header sheds its summary, act stepper and character/world chips while writing. The text **fills the sheet** (padding only keeps it off the border) — an early 660px centred column left ~390px of dead space each side and read as broken. One `Edit` / `View` toggle, no formatting toolbar: the editor is deliberately plain because the *coupling* is the product. View starts on Edit every time and is not persisted — unlike the three states it has a wrong side to land on. |
...
| Timeline | Read the book as prose | ✅ | A `Scenes / Prose` **pane toggle** beside the ↓ / → control, not a fourth view: same rail, same cards, same active ring, same two-way scroll sync — only the pane's contents change. Renders through `ProsePane`, the same component the editor's View mode uses. Vertical fills the pane (the rail already takes the left of the window); horizontal keeps a fixed column, because there the pane scrolls sideways and every chapter needs the same width. |
