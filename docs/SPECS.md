...
└─ src/
   ...
   └─ components/
      ...
      ├─ ChapterModal.tsx     # picks the chapter's face, and owns the word-count recompute
      ├─ ChapterDetail.tsx    # the story map face: scene flow, act controls, reference tabs
      ├─ ChapterMeta.tsx      # the meta line both faces share, so the two cannot drift
      ├─ ManuscriptModal.tsx  # the writing pane, with the beats beside it
...
| Board | Connectors (therefore/but/and) | ✅ | SVG curves, colored by type, per-version (each version forks its own links). Chapter links share `ConnType` with scene links, so since v9 one *can* hold `"none"` (neutral line colour) — but the board has no cycle control and nothing here sets it; the fourth value is a scene-seam feature. |
...
| Detail | Act on several scenes at once | ✅ | **Select scenes** mode picks any number of beats, and offers the block two verbs: **move** it, or **delete** it. One mode, not two — the picking is identical either way, so a separate delete mode would have been the same mechanic under another name. A moved block can go **to another chapter or stay in this one**. `moveScenesWithin` is a separate action from `moveScenesToChapter` rather than a loop over `reorderScene`: each single hop renumbers everything after it, so the result of "move 2 and 5 to the front" would depend on the order the hops ran in. **`atIdx` counts against the scenes left after the selection is lifted out**, not the original list — the only unambiguous reading, since an index into the original can point at a scene that is itself moving, and it is what beginning / middle / end mean to someone looking at the chapter. Two ways in: the destination picker leads with **This chapter · reorder** (hidden when every scene is selected, where there is nothing to position against), and **dragging any selected card moves the whole block**. The drag arms on press and only becomes a drag once the pointer moves, because in select mode a click on a card toggles its selection and a drag that started on mousedown would deselect the card it was picked up by. The block's ghost is one card standing in for several; the selection follows the block to where it lands, since selection is by index and would otherwise highlight whatever now sits at the old positions. **Deleting** the block goes through `deleteScenes`, a sibling of the move actions and separate for the same reason — indices are read against the list the writer sees and resolved once, where repeated `deleteScene` would renumber under its own feet. Seams close via the shared `sceneSubset`, so a gap that closes over deleted scenes comes back unlabeled. Positions are filtered rather than re-arranged, so surviving cards stay where they were put (a hole in a hand-arranged canvas, which Auto-arrange clears). **Clearing every scene leaves the chapter with one blank scene**, the state a fresh chapter starts in and the one `moveScenesToChapter` leaves an emptied source in — and the confirm says so, because finding a blank scene you did not ask for should be the thing you were told would happen. The prose is untouched either way. |
...
| Notes | Images downscaled on import | ✅ | `readImageForStorage` in `lib/files.ts` (2026-08-18), used by both upload sites — image assets and book covers. Longest edge 2048, measured against the lightbox (92vw, then 1.6× on click) rather than picked round; JPEG at 0.85 **unless** the source can carry alpha and does, since a transparent PNG re-encoded as JPEG comes back on a black background. GIF and SVG never touch a canvas (one would lose its animation, the other its vectors). An image already within the edge and under 1MB is stored byte-for-byte. It never rejects: an undecodable format, a canvas that will not paint, or a re-encode that comes out bigger all fall back to the original data URL — this is an optimisation, not a gate. The reason it matters after the pictures left localStorage is that the *document* is still what gets exported, synced and read by the Android app. |
...
| Persist | Local auto-save | ✅ | Via zustand persist → LocalStorageAdapter (debounced; failures surfaced in footer). **Split at rest, whole everywhere else:** the map goes to localStorage on a 500ms trailing timer, while manuscripts (`store/prose.ts`) and pictures (`store/images.ts`, 2026-08-18) go to IndexedDB on a 200ms one, sharing one database via `store/idb.ts`. `StoryDoc` is reassembled on load, so the store, exports, Sync and the Android contract only ever see one whole document — the ~5MB blob carries the map and nothing else. A blob written this way is marked `payloadsExternal`, and a load that cannot reach IndexedDB refuses rather than showing blank chapters and missing covers. Prose has a synchronous localStorage crash pad (IndexedDB is async, `beforeunload` is not); pictures deliberately do not, since megabytes of base64 in the pad would recreate the quota failure the split removes. |
...
### Installing Estoria as an app (added 2026-08-06)

Estoria is installable: Chrome, Edge and Safari can give it its own window, its
own icon, and offline loading. Nothing about the data changes — an installed
copy is the same origin reading the same browser storage as the tab.
...
| `art/icon-source.png` | Ray's artwork, 1024², the one source of truth. Outside `public/` so the master isn't served |
| `public/icon-*.png`, `favicon-32.png`, `apple-touch-icon.png` | Generated by `scripts/make-icons.sh`, committed |
| `public/sw.js` | Offline app shell |
| `src/lib/install.ts` | Captures `beforeinstallprompt`, detects standalone |
| `src/lib/sw.ts` | Registration + the update handshake |
| `src/components/modals/InstallModal.tsx` | File → Install Estoria |
| `src/components/UpdateToast.tsx` | "A new version is ready" |

**Two path decisions, both about `base`.** Vite rewrites root-absolute URLs in
`index.html` with the build's base, so `/icon.svg` becomes `/estoria/icon.svg`
in production on its own. The manifest is *not* processed by Vite (nothing in
`public/` is), so every URL inside it is **relative** instead — a relative
`start_url`/`scope`/icon `src` resolves against the manifest's own URL, which
gives `/` in dev and `/estoria/` in prod without a build step knowing anything.

**The worker's version is the build number.** `sw.js` is static, so it can't
have a build stamped into it. The page registers it as `sw.js?v=<build>` using
the same commit count the About dialog shows: a new commit means a new script
URL, which is what makes the browser install a new worker, and the query value
is what names the cache inside it. One version source, nothing extra to bump.

**Caching rules** (`public/sw.js`): hashed `/assets/*` cache-first (immutable by
construction); navigations network-first with the cached shell as the offline
fallback; Google Fonts cache-first in a cache that survives builds;
**`version.json` is never cached** — it's what a deploy is verified against.
On install the worker reads `index.html` and precaches the hashed assets it
finds, so offline works from the *first* visit rather than the second.

**Updates are offered, not applied.** The worker never calls `skipWaiting()` on
install. A new build sits in `waiting` and `UpdateToast` offers a reload;
...
ways out exist before they're needed:

| | What | When |
| --- | --- | --- |
| Fleet-wide | `KILL_SWITCH = true` at the top of `public/sw.js`, then commit + `npm run deploy` | A cached shell is bad for everyone |
| One person | `…/estoria/?sw=off` (and `?sw=on` to allow it again) | One person is stuck; a link you can paste into a reply |

The fleet-wide switch reaches clients even when the cached shell is broken,
because the browser always re-fetches `sw.js` from the network on navigation —
a cache can't hide the switch from itself. An armed worker skips waiting, drops
every `estoria-` cache, unregisters, and answers no requests, so the next load
is an ordinary uncontrolled one. It does **not** reload anybody: the session in
front of the user stays up.

`?sw=off` is deliberately **sticky** (a `estoria:sw-disabled` flag in
localStorage). Without that, the reload at the end of the teardown would
re-register the worker on the way back in — a fine cache repair, but useless to
someone the worker itself is breaking. It runs from `main.tsx` before React
renders, since the shell it's rescuing someone from may be why the app won't
mount.

**The card fills the frame.** The source carries a wide transparent margin, so
a straight downscale leaves the card small and adrift in its own canvas. Every
size is trimmed to the artwork first, then padded back to square on the long
edge (718px) so the card touches the left and right edges without stretching.
The icon *is* the card, not a card sitting on something.
...
diagonal from centre — not the corners of its bounding box, which are empty.
At 370 that distance is ~196px. Apple's gets a fuller 153-of-180, since iOS
applies only its own rounded rect.

Regenerate with `scripts/make-icons.sh` (`npm run icons`) after changing
