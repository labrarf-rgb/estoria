...
| Detail | Manuscript: write inside the chapter | ✅ | **The prose lives in the map.** A `Manuscript` section under Scene flow, and an **ordinary collapsible section of the chapter modal** like Characters or Chapter notes — collapsed by default, so a planning session never has to look at prose. Open, it carries `Edit` / `View` and an `Expand` / `Collapse` for size, exactly the pair the scene canvas has; expanding also widens the modal. It is deliberately *not* a mode: an earlier version had a three-way Minimized / Regular / Full screen control that took the modal over, and it read as a separate application inside the chapter. **Scene flow is collapsible too** — it was the only section that wasn't. Above the sheet sits the beat guide: a sticky, horizontally scrolling strip of the chapter's beats and their connector pills, each card a **fixed height that grows sideways** with its text, because a card that grows downwards makes the guide as tall as its wordiest beat and pushes the writing off the screen. The text fills the sheet; an early 660px centred column left ~390px of dead space each side and read as broken. |
